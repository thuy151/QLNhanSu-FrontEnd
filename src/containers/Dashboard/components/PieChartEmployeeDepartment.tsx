...
function PieChartEmployeeDepartment(props: any) {
    ...
    const getDataDepartment = useCallback(async () => {
        ...
        if (resp?.status === 200) {
            setDataDepartment(data?.content)
            const color:any =[];
    ...
    const formatDataNotication = useCallback((obj: any, key: string) => {
        ...
        for (const property in obj) {
            ...
            if (a[key] === 'total') {
                ...
                a[key] = data?.[property]?.[0]?.department?.name 
            }
            dataFormated.push(a);
        }
        return dataFormated;
    ...
    }, [data, dataDepartment, formatDataNotication]);
        ...
        return (
            <g>
                <text x={cx} y={cy - 15} fontSize={17} textAnchor="middle" fill={fill}>
                    {payload.name === 'Truy cứu trách nhiệm hình sự' ? `Truy cứu TNHS` : payload.name}: {`${value}`}
...
export default PieChartEmployeeDepartment